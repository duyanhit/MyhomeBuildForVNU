...
class DanhSachThongBao extends AppComponent {
  constructor(props) {
    super(props);
    this.state = {
      ...this.state,
  ...
  getScreenData = () => {
    ...
    if (accountInfo) {
      this.getFromServerWithAccount(getApiUrl(API.DS_THONG_BAO), {
  ...
  renderItem = ({ item, index }) => {
    return (
      <InAppItem
        ...
        icon={"ios-mail"}
        api={API.XOA_THONG_BAO}
        time={item.sent_at}
        message={string.xoaThongBaoDaChon}
      />
    );
  };

  /**
   * sang màn hình chi tiết thông báo
   * @param item
   */
  toChiTietScreen = item => {
    BackHandler.removeEventListener("hardwareBackPress", this.handleBackPress);
    this.navigateToScreen(screenNames.ChiTietThongBao, {
      id: item.id,
      onGoBack: () => {
        BackHandler.addEventListener("hardwareBackPress", this.handleBackPress);
      },
  ...
  render() {
    ...
    if (countHome === 0) {
      ...
      contentView = this.renderView(propsData, typeList.THONGBAO);
    }
    ...
    return (
      <Container style={{ backgroundColor: colors.windowBackground }}>
        <InAppHeader
          ...
          api={API.XOA_THONG_BAO}
          message={string.xoaCacThongBaoDaChon}
        />
